...
Write a function that detects if a number is Armstrong number in functionality style:
...
def is_armstrong(number: int) -> bool:
    return reduce(lambda x, y: int(x) + pow(int(y), len(str(number))), str(number), 0) == number
